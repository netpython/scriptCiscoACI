...
from common import *
log=setup("interface_policy_compliance");a=parser("Conformité interfaces ACI").parse_args();result=[]
with APIC(config(a.config)) as apic:
 phys={r.get("dn"):r for r in rows(apic.cls("l1PhysIf"))};oper=rows(apic.cls("ethpmPhysIf"));errors=rows(apic.cls("rmonEtherStats"))
errmap={r.get("dn","").split("/dbgEtherStats")[0]:r for r in errors}
for r in oper:
 dn=r.get("dn","").replace("/phys","" );base=next((p for p in phys.values() if p.get("id") and p.get("id") in dn),{});err=next((e for k,e in errmap.items() if k in dn),{});issues=[]
 if r.get("operSt")!="up":issues.append("OPER_DOWN")
 if not base.get("descr"):issues.append("NO_DESCRIPTION")
 ...
 if error_total:issues.append("PHYSICAL_ERRORS")
